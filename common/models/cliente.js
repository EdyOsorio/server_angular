...
var debug = require('debug')('loopback:cliente');

/**
 * Built-in Clientes model.
 * Extends LoopBack [PersistedModel](#persistedmodel-new-persistedmodel).
 *
 * Default `Clientes` ACLs.
 *
 * - DENY EVERYONE `*`
...
module.exports = function(Cliente) {
 ...
 /**
   ...
   *
   */
  Cliente.prototype.createAccessToken = function(ttl, options, cb) {
   ...
   */
  Cliente.normalizeCredentials = function(credentials, realmRequired, realmDelimiter) {
    var query = {};
    ...
    if (!realmRequired) {
      ...
      if (credentials.email) {
        ...
        parts = splitPrincipal(credentials.nombre, realmDelimiter);
        query.nombre = parts[1];
        if (parts[0]) {
          query.realm = parts[0];
        }
      }
  ...
  /**
   * Login a cliente by with the given `credentials`.
   *
   * ```js
   *    Cliente.login({nombre: 'foo', password: 'bar'}, function (err, token) {
   *      console.log(token.id);
   *    });
   * ```
   *
   * If the `emailVerificationRequired` flag is set for the inherited cliente model
   * and the email has not yet been verified then the method will return a 401
   * error that will contain the cliente's id. This id can be used to call the
   * `api/verify` remote method to generate a new email verification token and
   * send back the related email to the cliente.
   *
   * @param {Object} credentials nombre/password or email/password
   * @param {String[]|String} [include] Optionally set it to "cliente" to include
   * the cliente info
  ...
  Cliente.login = function(credentials, include, fn) {
    var self = this;
    ...
    self.findOne({where: query}, function(err, cliente) {
      ...
      function tokenHandler(err, token) {
        ...
        if (Array.isArray(include) ? include.indexOf('cliente') !== -1 : include === 'cliente') {
          ...
          token.__data.cliente = cliente;
        }
        fn(err, token);
      }

      if (err) {
        debug('An error is reported from Clientes.findOne: %j', err);
        fn(defaultError);
      } else if (cliente) {
        cliente.hasPassword(credentials.password, function(err, isMatch) {
          if (err) {
            debug('An error is reported from Clientes.hasPassword: %j', err);
            fn(defaultError);
          } else if (isMatch) {
            if (self.settings.emailVerificationRequired && !cliente.emailVerified) {
              ...
              if (cliente.createAccessToken.length === 2) {
                cliente.createAccessToken(credentials.ttl, tokenHandler);
              } else {
                cliente.createAccessToken(credentials.ttl, credentials, tokenHandler);
              }
            }
          } else {
            debug('The password is invalid for clientes %s', query.email || query.nombre);
            fn(defaultError);
          }
        });
      } else {
        debug('No matching record is found for clientes %s', query.email || query.nombre);
        fn(defaultError);
      }
    });
    return fn.promise;
  ...
  *    });
  ...
  Cliente.logout = function(tokenId, fn) {
    fn = fn || utils.createPromiseCallback();
  ...
  Cliente.observe('before delete', function(ctx, next) {
    ...
    ctx.Model.find({where: ctx.where, fields: [pkName]}, function(err, list) {
      ...
      AccessToken.destroyAll({clienteId: {inq: ids}}, next);
    });
  });
  ...
  Cliente.prototype.hasPassword = function(plain, fn) {
    fn = fn || utils.createPromiseCallback();
   ...
   *
   * @param {*} clienteId Id of the users changing the password
   ...
   * @promise
   */
  Cliente.changePassword = function(clienteId, oldPassword, newPassword, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
    ...
    this.findById(clienteId, options, (err, inst) => {
      if (err) return cb(err);

      if (!inst) {
        const err = new Error(` Cliente ${clienteId} not found`);
        Object.assign(err, {
   ...
   * @promise
   */
  Cliente.prototype.changePassword = function(oldPassword, newPassword, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
  ...
  Cliente.datosFactura = function(direccion, identificacion, telefono, id, cb) {
    var sql = 'UPDATE cliente SET direccion = ?, identificacion = ?, telefono = ? WHERE id = ?';
    var params = [direccion, identificacion, telefono, id];
    Cliente.dataSource.connector.query(sql, params, cb)
  };
   ...
   * @promise
   */
  Cliente.setPassword = function(clienteId, newPassword, options, cb) {
    assert(clienteId != null && clienteId !== '', 'clienteId is a required argument');
   ...
   * @promise
   */
  Cliente.prototype.setPassword = function(newPassword, options, cb) {
    assert(!!newPassword, 'newPassword is a required argument');
  ...
  Cliente.getVerifyOptions = function() {
  ...
  Cliente.prototype.verify = function(verifyOptions, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
    ...
    Cliente.generateVerificationToken;
    ...
    if (!verifyOptions.verifyHref) {
      ...
      if (!confirmMethod) {
        throw new Error(
          'Cannot build clientes verification URL, ' +
      ...
      verifyOptions.verifyHref =
        ...
        urlPath +
        '?' + qs.stringify({
          uid: '' + verifyOptions.cliente[pkName],
          redirect: verifyOptions.redirect,
        });
    }

    verifyOptions.to = verifyOptions.to || cliente.email;
    ...
    if (tokenGenerator.length == 3) {
      tokenGenerator(cliente, options, addTokenToUserAndSave);
    } else {
      tokenGenerator(cliente, addTokenToUserAndSave);
    }

    function addTokenToUserAndSave(err, token) {
      if (err) return cb(err);
      cliente.verificationToken = token;
      cliente.save(options, function(err) {
        if (err) return cb(err);
        sendEmail(cliente);
      });
    }
    ...
    function sendEmail(cliente) {
      verifyOptions.verifyHref +=
        verifyOptions.verifyHref.indexOf('?') === -1 ? '?' : '&';
      verifyOptions.verifyHref += 'token=' + cliente.verificationToken;

      verifyOptions.verificationToken = cliente.verificationToken;
      ...
      function setHtmlContentAndSend(err, html) {
        ...
        function handleAfterSend(err, email) {
          if (err) return cb(err);
          cb(null, {email: email, token: cliente.verificationToken, uid: cliente[pkName]});
        }
      }
    }
   ...
   *
   * @param {object} clientes The User this token is being generated for.
   * @param {object} options remote context options.
   * @param {Function} cb The generator must pass back the new token with this function call.
   */
  Cliente.generateVerificationToken = function(cliente, options, cb) {
   ...
   * @promise
   */
  Cliente.confirm = function(uid, token, redirect, fn) {
    fn = fn || utils.createPromiseCallback();
    this.findById(uid, function(err, cliente) {
      if (err) {
        fn(err);
      } else {
        if (cliente && cliente.verificationToken === token) {
          cliente.verificationToken = null;
          ...
          if (cliente) {
            ...
            err = new Error(g.f('Clientes not found: %s', uid));
            err.statusCode = 404;
  ...
  Cliente.resetPassword = function(options, cb) {
    cb = cb || utils.createPromiseCallback();
    var UserModel = this;
    ...
    UserModel.findOne({where: where}, function(err, cliente) {
      if (err) {
        return cb(err);
      }
      if (!cliente) {
      ...
      if (UserModel.settings.emailVerificationRequired && !cliente.emailVerified) {
      ...
      if (UserModel.settings.restrictResetPasswordTokenScope) {
        ...
        cliente.createAccessToken(tokenData, options, onTokenCreated);
      } else {
        ...
        cliente.createAccessToken(ttl, onTokenCreated);
      }
  ...
  Cliente._invalidateAccessTokensOfUsers = function(clienteIds, options, cb) {
    ...
    var query = {clienteId: {inq: clienteIds}};
    ...
    var relatedUser = AccessToken.relations.cliente;
  ...
  Cliente.setup = function() {
    // We need to call the base class's setup method
    Cliente.base.setup.call(this);
    var UserModel = this;
    ...
    UserModel.beforeRemote('create', function(ctx, cliente, next) {
    ...
    UserModel.remoteMethod(
      'login',
      {
        description: 'Login a Cliente with nombre/email and password.',
        accepts: [
        ...
        returns: {
          ...
          description:
            ...
            '  - ` cliente` - `U+007BUserU+007D` - Data of the currently logged in cliente. ' +
            '{{(`include=users`)}}\n\n'),
        },
        http: {verb: 'post'},
      }
    ...
    UserModel.remoteMethod(
      'logout',
      {
        description: 'Logout a cliente with access token.',
        accepts: [
    ...
    UserModel.remoteMethod(
      'prototype.verify',
      {
        description: 'Trigger cliente\'s identity verification with configured verifyOptions',
        accepts: [
    ...
    UserModel.remoteMethod("datosFactura", {
      returns: {
        arg: "cliente",
      ...
      description: [
        'Actualiza datos de cliente para la factura',
      ],
      accepts: [
        {
          arg: 'direccion',
    ...
    UserModel.remoteMethod(
      'setPassword',
      {
        description: 'Reset cliente\'s password via a password-reset token.',
        accepts: [
          {arg: 'id', type: 'any', http: getUserIdFromRequestContext},
          {arg: 'newPassword', type: 'string', required: true, http: {source: 'form'}},
          {arg: 'options', type: 'object', http: 'optionsFromRequest'},
    ...
    // default models
    assert(loopback.Email, 'Email model must be defined before cliente model');
    UserModel.email = loopback.Email;

    assert(loopback.AccessToken, 'AccessToken model must be defined before cliente model');
    ...
    if (UserModel.settings.realmRequired && UserModel.settings.realmDelimiter) {
      ...
      // Regular(Non-realm) clientes validation
      UserModel.validatesUniquenessOf('email', {message: 'Email already exists'});
      UserModel.validatesUniquenessOf('nombre', {message: 'Cliente already exists'});
    }
  ...
  Cliente.observe('access', function normalizeEmailCase(ctx, next) {
  ...
  Cliente.observe('before save', function rejectInsecurePasswordChange(ctx, next) {
    ...
    if (ctx.isNewInstance) {
      // The password can be always set when creating a new Cliente instance
      return next();
    }
    ...
    // when calling `this.patchAttritubes()` to change cliente's password
    ...
    const err = new Error(
      'Changing cliente password via patch/replace API is not allowed. ' +
  ...
  Cliente.observe('before save', function prepareForTokenInvalidation(ctx, next) {
    ...
    ctx.Model.find({where: where}, ctx.options, function(err, userInstances) {
      ...
      ctx.hookState.originalUserData = userInstances.map(function(u) {
        var cliente = {};
        cliente[pkName] = u[pkName];
        cliente.email = u.email;
        cliente.password = u.password;
        return cliente;
      });
      var emailChanged;
      if (ctx.instance) {
  ...
  Cliente.observe('after save', function invalidateOtherTokens(ctx, next) {
